...
public class Search2 {
	...
	static void writeIndex(RAMDirectory ramDir, Analyzer analyzer) {
		try {
			...
			indexDoc(writer, "document-1", "Merhaba Dünya");
			indexDoc(writer, "document-2", "dünya çölleþiyor");
			indexDoc(writer, "document-3", "Merhaba Ankara");
			indexDoc(writer, "document-4", "Çölleþme");
	...
	static void searchIndex( Analyzer analyzer) {
		...
		try {
			...
			// Build query
			QueryParser qp = new QueryParser("headline", analyzer);
			Query query = qp.parse("Astroloji");
